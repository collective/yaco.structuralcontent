...
class ContentControl(BrowserView):
    ...
    def allowLock(self):
        """
        """
        return IContentish.providedBy(self.context) \
...
class LockForm(form.Form):
    ...
    description = _(u"To lock this object as 'Structural content' click on 'Save'. "
...
class UnlockForm(form.Form):
    ...
    description = _(u"To unlock this 'Structural content' object click on 'Save'. "
    ...
    def handleApply(self, action):
        ...
        self.context.reindexObject(idxs=['object_provides'])
        plone_utils.addPortalMessage(_(u"The object was unlocked and can now could be modified"))
        self.request.response.redirect(self.context.absolute_url())
